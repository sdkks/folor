...
if command -v cargo-llvm-cov &> /dev/null; then
    ...
    cargo llvm-cov --fail-under-lines 80 --fail-under-functions 80 --ignore-filename-regex '(main\.rs|config\.rs|binary\.rs|file_ref\.rs|signal\.rs)' || { echo "Coverage below 80%"; exit 1; }
else
